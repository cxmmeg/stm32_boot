...
static int32_t Receive_NByte(uint8_t *buf, uint32_t len, uint32_t timeout)
{
  ...
  if (status == HAL_OK)
  {
    switch (char1)
    {
      ...
      case EOT:
        break;
      case CA:
        if ((Receive_Byte(&char1, timeout) == 0) && (char1 == CA))
        {
          packet_size = 2;
        }
        else
    ...
    *p_data = char1;
    ...
    {
      status = Receive_Byte( &p_data[PACKET_NUMBER_INDEX], packet_size + PACKET_OVERHEAD_SIZE, timeout);
